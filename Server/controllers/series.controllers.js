...
exports.create = (req, res) => {
  ...
  Series.save(series)
    ...
    .catch((err) => {
      res.status(500).send({
        message: err.message || 'some error occured while creating the serie',
      });
    });
};
...
exports.findAll = (req, res) => {
  Series.find()
    ...
    .catch((err) => {
      res.status(500).send({
        message: err.message || 'some error ocuured while retrieving series',
      });
    });
};
  ...
  Series.findById(req.params.id)
    .then((data) => {
      if (!data) {
        res
          .status(404)
          .send({ message: 'Not found serie with id' + req.params.id });
      } else {
        res.send(data);
      }
    })
    .catch((err) => {
      res
        .status(500)
        .send({ message: 'Error Retrieving serie with id ' + req.params.id });
    });
};
