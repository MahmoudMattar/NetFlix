...
exports.findOne = (req, res) => {
  Films.findById(req.params.id)
    .then((data) => {
      if (!data) {
        res.status(404).send({ message: "Not found series with id" + req.params.id });
      } else {
        res.send(data);
      }
    })
    .catch((err) => {
      res.status(500).send({ message: "Error Retrieving series with id " + req.params.id });
    });
};
...
exports.update = (req, res) => {
  ...
  Films.findByIdAndUpdate(id, req.body, { useFindAndModfiy: false }).then((data) => {
    if (!data) {
      res
        .status(404)
        .send({
        ...
        .catch((err) => {
          res.status(500).send({ message: `ErrorUpdating films with id : ${id}` });
        });
    } else {
...
exports.deleteFilm = (req, res) => {
  const id = req.params.id;
  Films.findByIdAndRemove(id, { useFindAndModfiy: false })
    .then((data) => {
      if (!data) {
